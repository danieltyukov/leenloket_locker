...
bool checkRFID(String UID_To_Be_Checked) {
    if (nfc.tagPresent())
    {
        NfcTag tag = nfc.read();
        tag.print();
        ...
        Serial.println(UID);
        if (UID_To_Be_Checked.equals(UID)){
          return true;
        }
        else {
          return false;
...
void resetLCD(bool PIN = true){
  do {
    ...
    if(PIN) {u8g2.drawStr(1, 20, "Enter your PIN:");};
    
    u8g2.setFont(u8g2_font_smallsimple_tr);
    u8g2.drawStr(1, 63, "* Reset  |  # Enter"); 
...
void setup() {
  ...
  u8g2.firstPage();
  do {
  u8g2.drawXBMP( 0, 0, 128, 64, LeenLoket); } while ( u8g2.nextPage() );
